...
export function ChoroplethMapChart({
  ...
  const enrichedGeoJson = React.useMemo(() => {
    ...
    const features = geoJson.features.map((f: any) => {
      ...
      const nameVal = f.properties?.name
        ? String(f.properties.name).toLowerCase()
        : "";
      const matchedVal =
        (idField === "name" ? dataMap.get(nameVal) : dataMap.get(idVal)) ??
        null;
    ...
    >
      ...
      >
        ...
        {pins.map((pin, i) => (
          <MapMarker
            key={i}
            longitude={pin.coord[0]}
            latitude={pin.coord[1]}
            anchor="bottom"
          >
