            ...
            >
              <div className="flex min-w-0 flex-1 items-center gap-1.5 pr-2">
                ...
                {showAssignee && task.assignee ? (
                  <span
                    className="bg-muted text-muted-foreground inline-flex size-5 shrink-0 items-center justify-center rounded-full text-[10px]"
                    title={task.assignee.name}
                    aria-label={`Assigned to ${task.assignee.name}`}
                  >
                    {task.assignee.initials ?? task.assignee.name.slice(0, 1)}
                  </span>
                ) : null}
              ...
              {showPriority ? (
                <div className="flex w-12 shrink-0 items-center justify-center">
                  {task.priority ? (
                    <Flag
                      className={cn("size-3", priorityColors[task.priority])}
                    />
                  ) : null}
                </div>
              ) : null}
