...
function applyMuted(map: mapboxgl.Map, dark: boolean) {
  ...
  for (const l of styleLayers) {
    ...
    try {
      if (id === "background" || id === "land")
      ...
      else if (/water/.test(id) && l.type === "fill")
        ...
        /^road-(secondary|tertiary|street|minor|service|path|pedestrian)/.test(
          id,
        )
      ) {
        map.setLayoutProperty(id, "visibility", "none");
      } else if (
        /poi|transit|airport|natural-point|water-point|waterway-label|building/.test(
          id,
        )
      ) {
        map.setLayoutProperty(id, "visibility", "none");
      } else if (
        /road-label|settlement-major-label|settlement-minor-label|state-label/.test(
          id,
        ) &&
        l.type === "symbol"
      ) {
...
const MapComponent = React.forwardRef<MapRef, MapProps>(
  (
    ...
    const resolvedToken =
      accessToken === ""
        ? ""
        : accessToken || configuredToken || DEFAULT_MAPBOX_TOKEN;
    ...
    React.useEffect(() => {
      ...
      if (!el) {
        const inViewTimer = window.setTimeout(() => setInView(true), 0);
        return () => window.clearTimeout(inViewTimer);
      }
    ...
    React.useImperativeHandle(ref, () => {
      void mapReady;
      return mapRef.current as MapRef;
    }, [mapReady]);
          ...
          >
            {navigation && (
              <MapboxNavigationControl
                ...
                showCompass={
                  showCompass !== undefined ? showCompass : (pitch ?? 0) > 0
                }
                showZoom={showZoom}
              />
            )}
            {fullscreen && (
              <MapboxFullscreenControl position={fullscreenPosition} />
            )}
            {children}
          </Map>
        )}
...
const MapMarkerComponent = React.forwardRef<mapboxgl.Marker, MapMarkerProps>(
  (
    { lngLat, ...props }: MapMarkerProps & { lngLat?: [number, number] },
    ref,
  ) =>
    lngLat ? (
      <MapboxMarker
...
function MapSourceComponent({
  id,
  options,
  ...rest
}: MapSourceProps & { options?: MapSourceOptions }) {
  return (
    <MapboxSource
      id={id}
      {...(options as object)}
      {...(rest as MapboxSourceProps)}
    />
  );
}
...
function MapLayerComponent({
  ...
  options,
  ...rest
}: MapLayerProps & { options?: MapLayerOptions }) {
...
export type { MapVariant } from "./map.variants";
