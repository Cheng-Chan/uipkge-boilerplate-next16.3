...
| FE12 — catalogue visibility — Complete | Expose every verified preview through a grouped, user-triggered gallery and clear homepage/catalogue entry points without loading preview code on `/`. | gallery route/feature, landing/catalogue links, tests                             | FE01, LAB04         | All verified items derive from snapshot metadata; previews load on demand and remain isolated. | Unit, static build, Playwright, network and linked-bundle inspection.           |
